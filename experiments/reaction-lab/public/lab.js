...
function onResult(m) {
  ...
  const row = {
    roundId: m.roundId, result: mine.result, R: mine.R,
    recorded: m.recorded, opponentR: other?.R ?? null, diff,
...
el.dl.addEventListener('click', () => {
  const cols = ['ts', 'roundId', 'result', 'R', 'recorded', 'opponentR', 'diff', 'recvToPaint',
